...
describe('MarkdownParser', () => {
  ...
  describe('CleanMarkdown', () => {
    it('Remove classes', () => {
      ...
      expect(mdParser.cleanMarkdown(html, { quizButton: 'check', codeboardButton: 'open IDE' })).to.be.equal('<div> hello<span>mist</span></div>');
    });
    it('Remove empty paragraphs', () => {
      const html = 'test <p></p> test <p>test</p> blub <p> </p>';
      expect(mdParser.cleanMarkdown(html, { quizButton: 'check', codeboardButton: 'open IDE' })).to.be.equal('test  test <p>test</p> blub ');
    });
  });
  ...
  describe('CodeMirror', () => {
    it('Render Button', () => {
      ...
      const result = '<div class="center"><a href="https://codeboard.io/projects/19870"\n' +
        ...
        ' Open IDE</a></div>';
      expect(mdParser.parseMarkdown(html, { quizButton: 'check', codeboardButton: 'Open IDE' })).to.be.equal(result);
    });
  });
});
